...
contract("TokenSale Contract", (accounts) => {
  ...
  it("Owner should be able to add User1,2 and 3 to the Vesting Schedule", async () => {
    ...
    const userVestingData_1 = await tokenInstance.userToVestingDetails(
      accounts[1],
      0
    );
    const userVestingData_2 = await tokenInstance.userToVestingDetails(
      accounts[2],
      0
    );
    const userVestingData_3 = await tokenInstance.userToVestingDetails(
      accounts[3],
      0
    );
    ...
    assert.equal(
      userVestingData_1[1],
      accounts[1],
      "Wallet Address Is wrongly assigned"
    );
    assert.equal(
      userVestingData_1[2].toString(),
      user1_amount.toString(),
      "Total Amount is wrongly assigned"
    );
    assert.equal(
      userVestingData_1[6].toString(),
      "5000000000000000000",
      "Vesting Rate Is wrongly assigned"
    );
    assert.equal(
      userVestingData_1[7].toString(),
      "0",
    ...
    assert.equal(
      userVestingData_2[1],
      accounts[2],
      "Wallet Address Is wrongly assigned for User 2"
    );
    assert.equal(
      userVestingData_2[2].toString(),
      user2_amount.toString(),
      "Total Amount is wrongly assigned for User 2"
    );
    assert.equal(
      userVestingData_2[6].toString(),
      "5000000000000000000",
      "Vesting Rate Is wrongly assigned  for User 2"
    );
    assert.equal(
      userVestingData_2[7].toString(),
      "0",
      "Total Amount claimed wrongly assigned for User 2"
    );
    assert.equal(
      userVestingData_2[8],
      true,
  ...
  it("Owner should be able to add User4,5,6 to the Sale Vesting Schedule", async () => {
    ...
    const userVestingData_4 = await tokenInstance.userToVestingDetails(
      accounts[4],
      7
    );
    const userVestingData_5 = await tokenInstance.userToVestingDetails(
      accounts[5],
      8
    );
    const userVestingData_6 = await tokenInstance.userToVestingDetails(
      accounts[6],
      9
    );
  ...
  it("Normal Vesting Rates should be calculated as expected", async () => {
    ...
    assert.equal(
      ...
      "Rates didn't match for User 1"
    );
    assert.equal(
      vestRate_2.toString(),
      expectedRate.toString(),
      "Rates didn't match for User 2"
    );
    assert.equal(
      vestRate_3.toString(),
      expectedRate.toString(),
      "Rates didn't match for User 3"
    );
  });
  ...
  it("Calculate Claimable Tokens function should work as expected before increasing time ", async () => {
    ...
    const actualClaimableReturn_user1 = await tokenInstance.calculateClaimableTokens(
      accounts[1],
      0
    );
    const actualClaimableReturn_user6 = await tokenInstance.calculateClaimableTokens(
      accounts[6],
      9
    );

    assert.equal(
      actualClaimableReturn_user1.toString(),
      expectedClaimableTokens_user1.toString(),
      "Calcualted Rate is wrong for user 1"
    );
    assert.equal(
      actualClaimableReturn_user6.toString(),
      expectedClaimableTokens_user6.toString(),
      "Calcualted Rate is wrong for user 6"
    );
  });
  ...
  it("Calculate Claimable Tokens function should work as expected before 60 days cliff ", async () => {
    ...
    const actualClaimableReturn_user4 = await tokenInstance.calculateClaimableTokens(
      accounts[4],
      7
    );

    assert.equal(
      actualClaimableReturn_user4.toString(),
      expectedClaimableTokens_user4.toString(),
      "Calcualted Rate is wrong for user 4"
    );
  });
  /**
  ...
  it("User 4 should not be able to Claim Vest tokens before Cliff Period Time", async () => {
    try {
      await tokenInstance.claimVestTokens(accounts[4], 7, {
        from: accounts[4],
      });
    } catch (error) {
  ...
  it("User 4 should be able to claim After Claim Period", async () => {
    ...
    const actualClaim_user4 = await tokenInstance.calculateClaimableTokens(
      accounts[4],
      7
    );
    ...
    assert.equal(
      balanceAfter_user4.toString(),
      expectedClaims_user4.toString(),
      "No Toknes Transferred"
    );
    assert.equal(
      userVestingData_4[7].toString(),
      expectedClaims_user4.toString(),
      "Total Amount Claimed Didn't update"
    );
    assert.equal(
      userVestingData_4[8],
      true,
      "isVesting Boolean is false before than expected"
    );
    assert.equal(
      userVestingData_4[9],
      false,
      "tgeTokensClaimed Boolean is true"
    );
  });

  it("User 2 should not be able to claim before Claim Period", async () => {
    try {
      await tokenInstance.claimVestTokens(accounts[2], 0, {
        from: accounts[2],
      });
    } catch (error) {
      const invalidOpcode = error.message.search("revert") >= 0;
  ...
  it("User 2 should be able to claim After Claim Period", async () => {
    ...
    const actualClaim_user2 = await tokenInstance.calculateClaimableTokens(
      accounts[2],
      0
    );
    ...
    assert.equal(
      balanceAfter_user2.toString(),
      expectedClaims_user2.toString(),
      "No Toknes Transferred"
    );
    assert.equal(
      userVestingData_2[7].toString(),
  ...
  it("User 1 should be able to claim After Claim Period 4 months", async () => {
    ...
    const actualClaim_user1 = await tokenInstance.calculateClaimableTokens(
      accounts[1],
      0
    );
    ...
    assert.equal(
      balanceAfter_user1.toString(),
      expectedClaims_user1.toString(),
      "No Toknes Transferred"
    );
    assert.equal(
      actualClaim_user1.toString(),
      expectedClaims_user1.toString(),
      "Claim Rate doesn't match"
    );
    assert.equal(
      userVestingData_1[7].toString(),
      expectedClaims_user1.toString(),
    ...
    assert.equal(
      userVestingData_1[9],
      false,
      "tgeTokensClaimed Boolean is false"
    );
  });
  ...
  it("User 1 should be able to claim After Claim Period 5 months", async () => {
    ...
    assert.equal(
      balanceBefore_user1.toString(),
      currentBalanceUser1.toString(),
      "No Toknes Transferred"
    );
    assert.equal(
      balanceAfter_user1.toString(),
      totalClaims_user1.toString(),
      "No Toknes Transferred after"
    );
    assert.equal(
      actualClaim_user1.toString(),
      expectedClaims_user1.toString(),
  ...
  it("User 4 should be able to Claim TGE tokens after TGE Time", async () => {
    ...
    const balanceAfter_user4 = await tokenInstance.balanceOf(accounts[4]);
    assert.equal(
      balanceBefore_user4.toString(),
      user4_current_balance.toString(),
      "No Toknes Transferred"
    );
    assert.equal(
      balanceAfter_user4.toString(),
  ...
  it("User 3 should be able to claim After Claim Period", async () => {
    ...
    const actualClaim_user3 = await tokenInstance.calculateClaimableTokens(
      accounts[3],
      0
    );
    ...
    assert.equal(
      balanceAfter_user3.toString(),
      expectedClaims_user3.toString(),
      "No Toknes Transferred"
    );
    assert.equal(
      userVestingData_3[7].toString(),
      expectedClaims_user3.toString(),
      "Total Amount Claimed Didn't update"
    );
    assert.equal(
  ...
  it("User 3 should not be able to claim For VEST Tokens TWICE", async () => {
    try {
      await tokenInstance.claimVestTokens(accounts[3], 0, {
        from: accounts[3],
      });
    } catch (error) {
      const invalidOpcode = error.message.search("revert") >= 0;
