...
contract("TokenSale Contract", (accounts) => {
  ...
  it("Calculate Claimable Tokens after 90 days Cliff", async () => {
    ...
    const actualClaim_user5 = await tokenInstance.calculateClaimableTokens(
      accounts[5],
      8
    );
    const actualClaim_user6 = await tokenInstance.calculateClaimableTokens(
      accounts[6],
      9
    );
    ...
    assert.equal(
      ...
      actualClaim_user3.toString(),
      "Expected Claims didn't match correctly for User 4"
    );
    assert.equal(
      expectedClaims_user4.toString(),
      actualClaim_user4.toString(),
      "Expected Claims didn't match correctly for User 4"
    );
    assert.equal(
      expectedClaims_user5.toString(),
      actualClaim_user5.toString(),
      "Expected Claims didn't match correctly for User 5"
    );
    assert.equal(
      expectedClaims_user6.toString(),
      actualClaim_user6.toString(),
      "Expected Claims didn't match correctlyfor User 6"
    );
  });
  ...
  it("Calculate Claimable Tokens after 120 days Cliff", async () => {
    ...
    assert.equal(
      actualClaim_user3.toString(),
      expectedClaims_user3.toString(),
      "Expected Claims didn't match correctly for User 3"
    );
    assert.equal(
      actualClaim_user4.toString(),
      expectedClaims_user4.toString(),
  ...
  it("Calculate Claimable Tokens after Total Vesting Duration is over", async () => {
    ...
    assert.equal(
      ...
      "Expected Claims didn't match correctly"
    );
  });
